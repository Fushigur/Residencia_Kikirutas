...
function printManifest() {
  ...
  const body = `
    ...
    <h3>${rutaSel.value!.nombre}</h3>
    <p>Fecha: ${rutaSel.value!.fechaISO}</p>
    <p>Pedidos: <b>${totalPedidos.value}</b> &mdash; Sacos: <b>${totalSacos.value}</b> &mdash; Estimado: <b>$${totalEstimado.value.toFixed(2)}</b></p>
    <table>
      <thead>
        <tr>
    ...
    <div class="max-w-screen-2xl mx-auto px-4 py-8">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
        ...
        <div class="space-y-6">
          ...
          <div class="rounded-2xl bg-white border border-gray-100 shadow-sm p-5">
            ...
            <div class="space-y-3">
              <div>
               ...
               <div>
                <label class="block text-xs font-bold text-gray-700 uppercase tracking-wide mb-1">Nombre (Opcional)</label>
                <input v-model="nuevaRuta.nombre" placeholder="Ej. Ruta Sur - Candelaria"
                  class="w-full rounded-xl border-gray-200 bg-gray-50 px-3 py-2 text-sm text-gray-900 focus:bg-white focus:border-brand focus:ring-4 focus:ring-brand/5 transition-all" />
              </div>

              <div>
                <label class="block text-xs font-bold text-gray-700 uppercase tracking-wide mb-1">Fecha de Entrega</label>
                <input v-model="nuevaRuta.fechaISO" type="date"
                  class="w-full rounded-xl border-gray-200 bg-gray-50 px-3 py-2 text-sm text-gray-900 focus:bg-white focus:border-brand focus:ring-4 focus:ring-brand/5 transition-all" />
              </div>

              <button class="w-full rounded-xl bg-brand/90 hover:bg-brand text-white font-bold py-2.5 shadow-lg shadow-brand/20 transition-all mt-2" @click="crearRuta">
                Crear Ruta
              </button>
            </div>
          </div>
          ...
          <div class="rounded-2xl bg-white border border-gray-100 shadow-sm p-5 flex flex-col h-[600px]">
            ...
            <div class="flex-1 overflow-y-auto space-y-2 pr-2">
              <div v-for="r in listaRutas" :key="r.id"
                ...
                <div class="flex justify-between items-end text-xs text-gray-500">
                  ...
                  <span class="font-medium bg-gray-100 px-1.5 py-0.5 rounded text-gray-600">{{ r.pedidos.length }} pedidos</span>
                </div>
              </div>
            ...
            <div v-if="rutaSel" class="pt-4 mt-2 border-t border-gray-100 grid grid-cols-2 gap-2">
               <button class="rounded-lg bg-gray-100 text-gray-600 text-xs font-bold py-2 hover:bg-red-50 hover:text-red-600 transition-colors disabled:opacity-50"
                  :disabled="!canDeleteRuta" @click="eliminarRuta">
                  Eliminar (Vacía)
               </button>
                <button class="rounded-lg bg-gray-100 text-gray-600 text-xs font-bold py-2 hover:bg-red-50 hover:text-red-700 transition-colors"
        ...
        <div class="xl:col-span-2 space-y-6">
          ...
          <div ref="pendientesRef" class="rounded-2xl bg-white border border-gray-100 shadow-sm p-0 overflow-hidden flex flex-col max-h-[500px]">
            <div class="p-5 border-b border-gray-100 bg-gray-50 flex flex-col sm:flex-row items-center justify-between gap-4">
               <div>
               ...
               <div class="flex items-center gap-2 w-full sm:w-auto">
                 <select v-model="rutaDestinoId"
                  ...
                  <button class="whitespace-nowrap rounded-xl bg-blue-600 text-white text-sm font-bold px-4 py-2 hover:bg-blue-500 shadow-md shadow-blue-200 disabled:opacity-50 disabled:shadow-none"
                    ...
                    <tr v-for="p in pendientes" :key="p.id" 
                      ...
                      <td class="px-4 py-3 text-center">
                        <input type="checkbox" :checked="seleccion.has(p.id)"
                          ...
                          @click.stop @change="toggleSeleccion(p.id, ($event.target as HTMLInputElement).checked)" />
                      </td>
          ...
          <div v-if="rutaSel" class="rounded-2xl bg-white border border-gray-100 shadow-sm p-0 overflow-hidden flex flex-col">
             <div class="p-5 border-b border-gray-100 bg-gray-50 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                <div>
                   <h3 class="font-bold text-gray-900 text-lg flex items-center gap-2">
                     {{ rutaSel.nombre }}
                     <span class="text-xs font-normal text-gray-500 bg-white border border-gray-200 px-2 py-0.5 rounded-full">{{ formatFechaLarga(rutaSel.fechaISO) }}</span>
                   </h3>
                ...
                <div class="flex gap-2">
                  <button @click="printManifest" class="inline-flex items-center gap-1.5 px-3 py-2 bg-white border border-gray-200 rounded-lg text-xs font-bold text-gray-700 hover:bg-gray-50 hover:text-gray-900 transition-colors">
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z" /></svg>
                    Imprimir
                  </button>
                   <button @click="exportCsv" class="inline-flex items-center gap-1.5 px-3 py-2 bg-white border border-gray-200 rounded-lg text-xs font-bold text-gray-700 hover:bg-gray-50 hover:text-gray-900 transition-colors">
                     <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>
                    CSV
                  </button>
                  <button :disabled="!hayEnRuta" @click="marcarTodosEntregados" class="ml-2 inline-flex items-center gap-1.5 px-3 py-2 bg-emerald-600 rounded-lg text-xs font-bold text-white hover:bg-emerald-500 shadow-md shadow-emerald-200 disabled:opacity-50 disabled:shadow-none">
                     Marcar Entregados
                  </button>
                </div>
             </div>
                  ...
                  <tbody class="divide-y divide-gray-100">
                    <tr v-for="p in pedidosRuta" :key="p.id" class="hover:bg-gray-50">
                       ...
                       <td class="px-4 text-right">
                          <button class="text-xs text-red-600 font-bold hover:underline" @click="quitarPedido(p.id)">Quitar</button>
                       </td>
                    </tr>
                     <tr v-if="pedidosRuta.length === 0">
                      <td colspan="6" class="py-8 text-center text-gray-400">Esta ruta está vacía. Añade pedidos desde la lista de pendientes.</td>
                    </tr>
                  </tbody>
                </table>
             </div>
             ...
             <div class="border-t border-gray-100 bg-gray-50 p-4">
               ...
               <div class="flex flex-wrap gap-2">
                  <div v-for="r in porProducto" :key="r.producto" class="bg-white border border-gray-200 rounded-lg px-3 py-1.5 text-xs shadow-sm">
                     <span class="font-bold text-gray-800">{{ r.sacos }}</span> <span class="text-gray-500">x</span> <span class="text-gray-700">{{ r.producto }}</span>
                  </div>
               </div>
             </div>
          </div>
          
          <div v-else class="rounded-2xl border-2 border-dashed border-gray-200 p-10 flex flex-col items-center justify-center text-gray-400">
